...
By default a monitor only records its state as telemetry — every tick, including a firing, is queryable and chartable, but nothing is paged automatically. Ask the agent to **arm** a monitor when a firing should escalate: an armed monitor opens an incident and hands the firing to the agent, and notifications land in your `#onepatch-alerts` Slack channel.
