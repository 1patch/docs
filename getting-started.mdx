...
<Warning>
Send OpenTelemetry over **HTTP with JSON encoding** (`http/protobuf` is not accepted). Set your exporter protocol to `http/json`. The setup skill does this for you; if you wire the SDK by hand, set it explicitly, because protobuf payloads are rejected at ingest.
</Warning>
